...
var commands = {};
var aliases;
exports.CMDHandler = function(bot) {
  load_CMDs(bot);
};
...
function load_CMDs(bot) {
  ...
  for (let i = 0; i < CMDs.length; i++) {
    ...
    try {
      ...
      console.log(
        `[${time} PST][${pm2Name}]` +
          ` Improper setup of the ${CMDs[i]} command. : ${err}`
      );
    }
    if (CMD) {
      if (`commands` in CMD) {
        for (let j = 0; j < CMD.commands.length; j++) {
          if (CMD.commands[j] in CMD) {
            addCommand(bot, CMD.commands[j], CMD[CMD.commands[j]]);
            commandCount++;
          }
        }
      }
      if (`custom` in CMD) {
        for (let j = 0; j < CMD.custom.length; j++) {
          if (CMD.custom[j] in CMD) {
            addCustomFunc(bot, CMD[CMD.custom[j]]);
            otherFunc++;
          }
        }
      }

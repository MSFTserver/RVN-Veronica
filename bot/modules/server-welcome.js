    ...
    if (event.t === `MESSAGE_REACTION_ADD`) {
      ...
      let msg = channel.fetchMessage(event.d.message_id).then(msg => {
        ...
        if (
          ...
          msg.content !=
            `you will be assigned the role "fam" and you can then participate in the discussions`
        ) {
          var role = `fam`;
...
// 			let mappedArray = [[toSend[0], !1], ...toSend.slice(1).map((msg, idx) => [msg, reactions[idx]])];
...
// 				messages.push(`you will be assigned the role **"fam"** and you can then participate in the discussions`);
// 			return messages;
// 		}
// 	}
// };
